...
let DiscountEngineService = class DiscountEngineService {
    ...
    async calculateAutomaticDiscount({ storeId, subtotal, }) {
        const now = new Date();
        ...
        for (const discount of discounts) {
            ...
            switch (discount.type) {
                case client_1.DiscountType.percentage:
                    discountAmount = subtotal * ((discount.value ?? 0) / 100);
                    break;
                case client_1.DiscountType.fixed_amount:
                    discountAmount = discount.value ?? 0;
                    break;
                case client_1.DiscountType.free_shipping:
                    freeShipping = true;
                    break;
            }
