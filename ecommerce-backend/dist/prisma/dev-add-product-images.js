...
async function main() {
    ...
    for (const product of products) {
        ...
        const nextImages = demoUrls.filter((url) => !existingUrls.has(url)).slice(0, Math.max(0, 3 - product.images.length));
        if (nextImages.length === 0) {
            console.log(`Producto ${product.id} (${product.slug}) ya tiene ${product.images.length} imagen(es).`);
            continue;
        }
        ...
        console.log(`Producto ${product.id} (${product.slug}) actualizado con ${nextImages.length} imagen(es) demo.`);
    }
}
main()
    .catch((error) => {
...
//# sourceMappingURL=dev-add-product-images.js.map