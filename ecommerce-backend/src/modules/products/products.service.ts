    ...
    const product = await this.prisma.$transaction(async (tx) => {
      const product = await tx.product.create({
        data: {
          ...
          type: isGiftCardProduct(title) ? 'GIFT_CARD' : 'STANDARD',
          trackInventory: !isGiftCardProduct(title),
  ...
  private async createProductRecord(
    ...
    return tx.product.create({
      data: {
        ...
        type: isGiftCardProduct(data.title) ? 'GIFT_CARD' : 'STANDARD',
        trackInventory: !isGiftCardProduct(data.title),
        lowStockThreshold: data.lowStockThreshold ?? 3,
        storeId,
      },
    });
