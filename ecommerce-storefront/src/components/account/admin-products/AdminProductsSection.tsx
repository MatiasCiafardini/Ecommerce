...
export default function AdminProductsSection({
  ...
  const copyProductLink = async (product: Product) => {
    ...
    try {
      const url = new URL(`/product/${encodeURIComponent(product.slug)}`, window.location.origin);
      await navigator.clipboard.writeText(url.href);
      showToast("Enlace copiado");
    } catch {
      showToast("No se pudo copiar el enlace. Intentá nuevamente.");
    }
  };
                    ...
                    <div style={iconActionsStyle}>
                      ...
                      {product.published ? (
                        <button
                          type="button"
                          title="Copiar enlace"
                          aria-label={`Copiar enlace de ${product.title}`}
